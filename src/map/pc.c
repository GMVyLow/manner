...
int pc_reg_received(struct map_session_data *sd)
{
	...
	if( battle_config.channel_system_enable )
	{
		...
		if( pc_has_permission(sd,PC_PERM_CHANNEL_ADMIN) && (sd->channels&8) && server_channel[CHN_GAMEMASTER] )
...
static void pc_bonus_item_drop(struct s_add_drop *drop, const short max, uint16 nameid, uint16 group, int class_, int race, int rate)
{
	...
	if (nameid && !group && !(it = itemdb_exists(nameid))) {
