...
static int mob_delay_item_drop(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	while (ditem) {
		...
		map_addflooritem(&ditem->item_data,ditem->item_data.amount,
			...
			list->first_charid,list->second_charid,list->third_charid,0,4);
		ditem_prev = ditem;
		ditem = ditem->next;
	...
	if(!(type&2) && //No exp
		...
		for(i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++) {
			...
			if (map[m].flag.nobaseexp || !md->db->base_exp)
				...
				base_exp = (unsigned int)cap_value(md->db->base_exp * per * (bonus+battle_config.base_exp_rate_bonus+md->option.exp_boost)/100. * map[m].adjust.bexp/100., 1, UINT_MAX);
			...
			if (map[m].flag.nojobexp || !md->db->job_exp || md->dmglog[i].flag == MDLF_HOMUN) //Homun earned job-exp is always lost.
				...
				job_exp = (unsigned int)cap_value(md->db->job_exp * per * (bonus+vip_bonus+battle_config.job_exp_rate_bonus+md->option.exp_boost)/100. * map[m].adjust.jexp/100., 1, UINT_MAX);
			}
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP; i++) {
			...
			{ // Card Drop Announce on Vending Channel
				char message[128];
				sprintf(message, msg_txt(NULL,823), server_channel[CHN_VENDING]->name, mvp_sd->status.name, it->jname);
		...
		{ // process script-granted extra drop bonuses
			...
			for (i = 0; i < ARRAYLENGTH(sd->add_drop); i++) {
					...
					(sd->add_drop[i].class_ == CLASS_ALL || sd->add_drop[i].class_ == status->class_)) //Matched class
				{
					...
					mob_item_drop(md, dlist, mob_setdropitem(dropid,1), 0, drop_rate, aikillonly);
				}
			}
