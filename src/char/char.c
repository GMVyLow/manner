...
int mmo_char_tosql(int char_id, struct mmo_charstatus* p)
{
	...
	if (
		...
		(p->rename != cp->rename) || (p->robe != cp->robe) || (p->character_moves != cp->character_moves) || (p->unban_time != cp->unban_time) || (p->playtime != cp->playtime)
	)
	{	//Save status
...
int mmo_char_fromsql(int char_id, struct mmo_charstatus* p, bool load_everything)
{
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`save_map`,`save_x`,`save_y`,`partner_id`,`father`,`mother`,`child`,`fame`,`rename`,`delete_date`,`robe`,`moves`, `unban_time`,`playtime`,`faction_id`"
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 54, SQLDT_UINT,   &p->playtime, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 55, SQLDT_INT,	&p->faction_id, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
